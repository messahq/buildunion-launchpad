-- Drop old function and recreate with correct signature
DROP FUNCTION IF EXISTS public.match_obc_chunks(vector, float, int);
...
RETURNS TABLE (chunk_id uuid, chunk_text text, section_number text, section_title text, similarity float)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
