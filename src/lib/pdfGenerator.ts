...
export const buildProjectSummaryHTML = (data: {
  ...
  const materialsRows = editedItems.map(item => `
    <tr>
      <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; font-weight: 500;">${item.name}</td>
    ...
    <div style="background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border: 1px solid #93c5fd; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
      ...
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 20px;">
        <div style="background: white; border-radius: 8px; padding: 12px 16px; text-align: center;">
          ...
          <span style="font-size: 18px; font-weight: 700; color: #1e40af;">${photoData.area || 'N/A'} ${photoData.areaUnit || 'sq ft'}</span>
        </div>
        <div style="background: white; border-radius: 8px; padding: 12px 16px; text-align: center;">
          ...
          <span style="font-size: 18px; font-weight: 700; ${photoData.areaConfidence === 'high' ? 'color: #16a34a;' : photoData.areaConfidence === 'medium' ? 'color: #ca8a04;' : 'color: #dc2626;'}">${(photoData.areaConfidence || 'Unknown').toUpperCase()}</span>
        </div>
        <div style="background: white; border-radius: 8px; padding: 12px 16px; text-align: center;">
          ...
          <span style="font-size: 18px; font-weight: 700; color: #1e40af;">${photoData.surfaceType || 'Unknown'}</span>
        </div>
      </div>
      ${photoData.summary ? `<div style="margin-top: 16px; padding: 12px; background: white; border-radius: 8px; font-size: 12px; color: #475569;"><strong>AI Summary:</strong> ${photoData.summary}</div>` : ''}
    </div>
  ` : '';
    ...
    <body>
      <div style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: white; padding: 24px 32px; display: flex; justify-content: space-between; align-items: center;">
        ...
        <div style="text-align: right; background: rgba(255,255,255,0.2); padding: 12px 20px; border-radius: 8px;">
          <div style="font-size: 16px; font-weight: 700; letter-spacing: 1px;">${quoteNumber}</div>
          <div style="font-size: 11px; opacity: 0.9; margin-top: 2px;">Generated: ${currentDate}</div>
        </div>
      </div>
      ...
      <div style="padding: 24px 32px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
          <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px;">
            ...
            <div style="font-size: 14px; font-weight: 600; color: #1e293b;">${clientInfo.name || 'Client Name'}</div>
            <div style="font-size: 12px; color: #64748b; margin-top: 4px;">${clientInfo.address || 'Address not specified'}</div>
            <div style="font-size: 12px; color: #64748b;">${clientInfo.phone || ''} ${clientInfo.email ? '• ' + clientInfo.email : ''}</div>
          </div>
          <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px;">
            ...
            <div style="font-size: 12px; color: #64748b; margin-top: 4px;">Created: ${createdAt || currentDate}</div>
            <div style="font-size: 12px; color: #64748b;">Status: ${(status || 'DRAFT').toUpperCase()}</div>
          </div>
        </div>
          ...
          <div style="display: flex; justify-content: flex-end; margin-top: 20px;">
            <div style="width: 300px; background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); border: 2px solid #f59e0b; border-radius: 8px; padding: 16px;">
              ...
              ${taxBreakdown.map(tax => `
                <div style="display: flex; justify-content: space-between; padding: 8px 0; font-size: 13px; border-bottom: 1px solid rgba(0,0,0,0.1);">
                  <span style="color: #78350f;">${tax.name} (${(tax.rate * 100).toFixed(tax.name === 'QST' ? 3 : 0)}%) <span style="background: #fef3c7; color: #92400e; font-size: 9px; font-weight: 600; padding: 1px 4px; border-radius: 3px;">${regionShortName || 'ON'}</span></span>
        ...
        ${notes ? `
          <div style="margin-top: 24px; padding: 20px; background: #f8fafc; border-radius: 8px; border-left: 4px solid #f59e0b;">
            ...
            <p style="font-size: 12px; color: #64748b; white-space: pre-wrap; margin: 0;">${notes}</p>
          </div>
        ` : ''}
